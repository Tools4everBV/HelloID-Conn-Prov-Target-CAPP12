#################################################
# HelloID-Conn-Prov-Target-CAPP12-Disable
# PowerShell V2
#################################################
...
function Resolve-CAPP12Error {
    ...
    process {
        ...
        try {
            $httpErrorObj.FriendlyMessage = ($httpErrorObj.ErrorDetails | ConvertFrom-Json).error
        }
        catch {
...
try {
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditLogMessage = "Could not disable CAPP12 account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    }
    else {
        $auditLogMessage = "Could not disable CAPP12 account. Error: $($ex.Exception.Message)"
