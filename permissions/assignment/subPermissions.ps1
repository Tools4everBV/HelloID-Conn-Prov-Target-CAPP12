################################################################
# HelloID-Conn-Prov-Target-CAPP12-SubPermissions-Assignment
# PowerShell V2
################################################################
...
$SecondaryLookupKey = { $_.Title.Name } # Mandatory
...
try {
    ...
    $actionMessage = "granting assignment positions to account [$($actionContext.References.Account)]"
    ...
    foreach ($permission in $desiredPermissions.GetEnumerator()) {
        ...
        try {
            ...
            if (-not $currentPermissions.ContainsKey($permission.Name)) {
                $actionMessage = "granting assignment position [$($permission.Value) ($($permission.Name))] to account with AccountReference: [$($actionContext.References.Account)]"
                
                $body = [PSCustomObject]@{
                    user_code     = $actionContext.References.Account
                    position_code = $permission.Key
                    ends_on       = $null
                ...
                if (-not($actionContext.DryRun -eq $true)) {
                    ...
                    $outputContext.AuditLogs.Add([PSCustomObject]@{
                            ...
                            Message = "Granted assignment position [$($permission.Value) ($($permission.Name))] to account with AccountReference: [$($actionContext.References.Account)]"
                            IsError = $false
                        })
                }
                else {
                    Write-Information "[DryRun] Would grant assignment position [$($permission.Value) ($($permission.Name))] to account with AccountReference: [$($actionContext.References.Account)]"
                }
            }
        }
        catch {
    ...
    $actionMessage = "revoking assignment positions from account [$($actionContext.References.Account)]"
    ...
    foreach ($permission in $currentPermissions.GetEnumerator()) {
        ...
        try {
            if (-not $desiredPermissions.ContainsKey($permission.Name)) {
                $actionMessage = "revoking assignment position [$($permission.Value) ($($permission.Name))] from account with AccountReference: [$($actionContext.References.Account)]"
                ...
                if (-not($actionContext.DryRun -eq $true)) {
                    ...
                    $outputContext.AuditLogs.Add([PSCustomObject]@{
                            ...
                            Message = "Revoked assignment position [$($permission.Value) ($($permission.Name))] from account with AccountReference: [$($actionContext.References.Account)]"
                            IsError = $false
                        })
                }
                else {
                    Write-Information "[DryRun] Would revoke assignment position [$($permission.Value) ($($permission.Name))] from account with AccountReference: [$($actionContext.References.Account)]"
                }
            }
            else {
